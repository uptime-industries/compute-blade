...
<Tabs groupId="os" defaultValue="windows">
    ...
    OpenSSH is included in MacOS. No Installation required
    </TabItem>
    <TabItem value="linux" label="Linux">
        Many distributions come with OpenSSH included but if yours does not here is how to install it on so common distributions
...
```
Once authenticated you will be presented with a active bash terminal.
```txt
uptime@blade:~$
```
...
With SSH, public key authentication improves security considerably, freeing users form remembering complicated passwords.
