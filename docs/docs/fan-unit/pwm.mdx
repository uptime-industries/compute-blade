...
:::info
Only the Compute Blade connected to `Port A` of the Fan Unit can control the fan speed. However both blades can read the speed of the fan.
:::
...
Start by cloning the scripts from the [GitHub repository](https://github.com/uptime-lab/compute-blade-pwm-fan-control)
```bash
git clone https://github.com/uptime-lab/compute-blade-pwm-fan-control.git
...
Either script can be stopped at any time with `CTRL` + `C`

### read_fan_speed.py
To use the `read_fan_speed.py` script run the following command:
```bash
