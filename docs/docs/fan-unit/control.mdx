...
:::note
    When Using the STA Fan Unit the speed of the fan can only be controled from `port A`
:::
...
dtoverlay=uart5
```
Next install a console we us `cu`. Next modifiy the premions to `/dev/ttyAMA5`. Then we can connect over UART to the Fan Unit
```bash
sudo apt install cu
...
The Fan Unit will appreare as a drive on your computer download [Circuit Python](https://circuitpython.org/board/raspberry_pi_pico/) `.uf2` file and move into the drive.
The Drive then disapere and a newone named `CIRCUITPY` will be conected the [example code](https://github.com/uptime-industries/circuitpy-fan-unit) should be place here to run
