...
dtoverlay=uart5
```
Next install a console we us `cu`. Next modify the permission to `/dev/ttyAMA5`. Then we can connect over UART to the Fan Unit
```bash
sudo apt install cu
...
The Fan Unit will appear as a drive on your computer download [Circuit Python](https://circuitpython.org/board/raspberry_pi_pico/) `.uf2` file and move into the drive.
The Drive then disconnect and a reconnect as named `CIRCUITPY` will be connected the [example code](https://github.com/uptime-industries/circuitpy-fan-unit) should be place here to run
